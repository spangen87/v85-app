...
function ValueBadge({ value, active }: { value: number; active: boolean }) {
  if (!active)
    return <span className="text-xs text-gray-500 italic">inväntar streckning</span>;
  if (value >= 5)
    return (
      <span className="text-xs font-bold text-green-400 bg-green-900/40 px-1.5 py-0.5 rounded whitespace-nowrap">
        +{value} pp ★
      </span>
    );
  if (value >= 2)
    return (
      <span className="text-xs font-semibold text-green-400 bg-green-900/20 px-1.5 py-0.5 rounded whitespace-nowrap">
        +{value} pp
      </span>
    );
  if (value <= -5)
    return (
      <span className="text-xs text-red-400 bg-red-900/20 px-1.5 py-0.5 rounded whitespace-nowrap">
        {value} pp
      </span>
    );
  return (
...
export function AnalysisPanel({ starters, raceMeters, raceStartMethod }: AnalysisPanelProps) {
  ...
  return (
    <div className="mt-3 bg-indigo-950/50 border border-indigo-800/50 rounded-xl p-4">
      ...
      {!hasStreckning && (
        <p className="text-xs text-amber-400 bg-amber-900/20 rounded px-2 py-1 mb-2">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-indigo-400/60 border-b border-indigo-800/40">
          ...
          <tbody>
            {results.map((r) => {
              ...
              return (
                <tr
                  key={r.start_number}
                  className={`border-b border-indigo-800/20 ${isValue ? "bg-green-900/20" : ""}`}
                >
                  <td className="py-1.5 pr-2 text-gray-500 tabular-nums">{r.start_number}</td>
                  <td className="py-1.5 pr-2 font-medium text-gray-100">{r.horse_name}</td>
                  <td className="py-1.5 pr-2 text-right font-mono text-indigo-300 tabular-nums">
                    {r.calc_pct}%
                  </td>
                  <td className="py-1.5 pr-2 text-right font-mono tabular-nums">
                    {r.streckning_loaded ? (
                      <span className="text-purple-400">{r.streckning_pct}%</span>
                    ) : (
                      <span className="text-gray-600">–</span>
                    )}
                  </td>
                  <td className="py-1.5 pr-2 text-right text-gray-400 tabular-nums">
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-indigo-400/60">
